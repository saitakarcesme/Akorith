...
const restoredAuthority = new PairingAuthority({ persistedState: authority.exportState() })
assert.equal(restoredAuthority.authenticate(replayApproval.deviceToken, 40_002)?.id, replayApproval.device.id, 'persisted token digest authenticates after restart')
assert.ok(!JSON.stringify(restoredAuthority.exportState()).includes(replayApproval.deviceToken), 'persisted authority state never contains plaintext bearer tokens')
