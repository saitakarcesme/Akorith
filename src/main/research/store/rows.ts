...
export function rowToResearchJob(row: DbRow): ResearchJob {
  return {
    ...
    mode: row.research_mode === 'autoresearch' ? 'autoresearch' : 'evidence',
    ...
    experimentConfig: parseJson<AutoresearchExperimentConfig>(row.experiment_config_json),
    experimentState: parseJson<AutoresearchExperimentState>(row.experiment_state_json),
    ...
    activeElapsedMs: numberValue(row.active_elapsed_ms),
    activeAccountingAt: optionalNumber(row.active_accounted_at),
