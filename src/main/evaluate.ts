...
${code}`
  ...
  return `You are judging generated tests for Loopex ISAScore quality.
...
async function writePdf(evaluation: EvaluationRow, rows: TestRunRow[], pdfPath: string): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const scores = scorePayload(evaluation)
    doc.font('Helvetica-Bold').fontSize(20).fillColor('#111827').text('Loopex Evaluation Report', left(doc), doc.y, {
      width: contentWidth(doc)
    })
    ...
    doc.moveDown(0.8)
    addRule(doc)
    small(doc, 'PDF generated by Loopex Phase 8 using the single evaluation report template.')
    doc.end()
  })
}
